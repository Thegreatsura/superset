...
const DEFAULT_LAYOUT: LayoutNode = {
  ...
  children: [
    ...
    { type: 'pane', command: null },
    {
      ...
      ratio: [1, 1],
      children: [{ type: 'pane', command: null }, { type: 'pane', command: null }],
    },
  ],
}
...
class WorkspaceManager {
  ...
  async createScreen(
    ...
    try {
      ...
      const worktree = workspace.worktrees.find((wt) => wt.id === input.worktreeId)
      if (!worktree) {
  ...
  async delete(
    ...
    try {
      ...
      if (removeWorktree) {
        const worktreePath = worktreeManager.getWorktreePath(workspace.repoPath, workspace.branch)
   ...
   * Get a screen by ID
   */
  getScreen(workspaceId: string, worktreeId: string, screenId: string): Screen | null {
    const config = configManager.read()
